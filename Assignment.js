...
var expapp=express();
const mysqlconn=mysql.createConnection({host:'localhost',user:'root',password:'4047@',database:'mysql'});
...
function getEmployee(request,response){
    ...
    for(i=0;i<emplist.length;i++){
      var emp=emplist[i];
      var str =
      "<html> <body> <table border=2> <head><tr><th><b>Name</b></th><th><b>Salary</b></th><th><b>landline</b></th><th><b>mobileno</b></th></tr></head>";
    str += "<body><tr><td>" + emp.emp_name + "</td><td>" + emp.salary + "</td><td>" + emp.landline + "</td><td>" + emp.mobileno + "</td></tr>";
    str +='</body></html>';
...
function getDelete(request,response){
    req=request;
    res=response;
    mysqlconn.query('delete employee where emp_id=101',printEmployees);
